...
`notion-tool-diff-mark` is centered on cli tools work in Python. The technical target is: Package a Python local lab for diff analysis with framed sample traffic, bounds and ordering tests, and documented operating limits.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 6.
